...
UINT ABonkAudience::GetSumOfSpawnWeights() const
{
	UINT WeightSum = 0;
...
{
	if (bWasSuccessfullySetup == false)
		return;
	...
	FVector StartRandomPoint, EndRandomPoint;
...
void ABonkAudience::BeginPlay()
{
	...
	bWasSuccessfullySetup = GetShapeActors(SourcePoints, Targets);
	if (bWasSuccessfullySetup)
	{
